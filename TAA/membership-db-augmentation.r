...
ebo.df <- read.csv("/Users/travismcarthur/Desktop/TAA work/Pay restructuring/Miscellaneous docs on pay and UW budget/EBO data map-plan table.csv",
         stringsAsFactor = FALSE)
dept.map.df <- read.csv("/Users/travismcarthur/Desktop/TAA work/Pay restructuring/Miscellaneous docs on pay and UW budget/DepartmentMapping.csv",
         stringsAsFactor = FALSE)
udds.crosswalk.df <- read.csv("/Users/travismcarthur/Desktop/TAA work/Pay restructuring/Miscellaneous docs on pay and UW budget/UDDS_Crosswalk_to_Discipline_Areas.csv",
         stringsAsFactor = FALSE)
# I'm not sure about the vintage of all these docs.

names(ebo.df)[names(ebo.df)=="Plan.Descr"] <- "PLAN_DESCR"
names(dept.map.df)[names(dept.map.df)=="Code"] <- "Plan.Group"
dept.map.df$Plan.Group <- as.character(dept.map.df$Plan.Group)

ebo.df <- ebo.df[complete.cases(ebo.df[, c("Plan.Group", "PLAN_DESCR")]), ]
dept.map.df <- dept.map.df[complete.cases(dept.map.df[, c("Plan.Group", "UDDS")]), ]

dept.map.df <- dept.map.df[!duplicated(dept.map.df[, c("Plan.Group", "UDDS")]), ]
# dept.map.df <- dept.map.df[dept.map.df$Category=="MAJ", ]


enrollment.plan.key <- enrollment.df[, c("PRIMARY_ACADEMIC_GROUP", "PLAN_DESCR")]
enrollment.plan.key <- unique(enrollment.plan.key)

ebo.df <- merge(ebo.df, enrollment.plan.key, all.y=TRUE)


sort(unique(dept.map.df$School.College))
sort(unique(ebo.df$PRIMARY_ACADEMIC_GROUP))
ebo.df$School.College <- factor(ebo.df$PRIMARY_ACADEMIC_GROUP, 
       levels= c("ALS", "BUS", "EDU", "EGR", "HEC", "IES", "L&S", "MED", "MSN", "NUR", "PHM", "VET"),
       labels= c("Agricultural & Life Sciences", "Business", "Education", "Engineering",
            "Human Ecology", "Nelson Institute for Environmental Studies", "Letters & Science",
            "Medicine and Public Health", "Letters & Science", "Nursing", "Pharmacy", "Veterinary Medicine")
)

ebo.df$School.College <- as.character(ebo.df$School.College)

# table(enrollment.plan.key$PLAN_DESCR %in% ebo.df$PLAN_DESCR)
# sort(unique(enrollment.plan.key$PLAN_DESCR))

dept.deg.df <- merge(ebo.df, dept.map.df, all=TRUE)
dept.deg.df  <- merge(dept.deg.df, udds.crosswalk.df, all=TRUE)

dept.deg.df <- dept.deg.df[!duplicated(dept.deg.df[, c("Department.Name", "PLAN_DESCR", "Discipline.Area.Code" )]),  ]
dept.deg.df <- dept.deg.df[complete.cases(dept.deg.df[, c("Department.Name", "PLAN_DESCR", "Discipline.Area.Code" )]),  ]

# table(duplicated(dept.deg.df$PLAN_DESCR))
# View(dept.deg.df[order(dept.deg.df$PLAN_DESCR), ])
# View(dept.deg.df[duplicated(dept.deg.df$PLAN_DESCR) | duplicated(dept.deg.df$PLAN_DESCR, fromLast = TRUE), ])

# Below is a by-hand fix (ugh)
dept.deg.index.to.exclude <- with(dept.deg.df,
  grepl("Scandinavian Studies", PLAN_DESCR) & Department.Name == "SOCIOLOGY" |
  grepl("Portuguese", PLAN_DESCR) & Department.Name == "GENDER AND WOMEN'S STUDIES" |
  grepl("History", PLAN_DESCR) & Department.Name == "CURRICULUM AND INSTRUCTION" |
  grepl("Communication Arts", PLAN_DESCR) & Department.Name == "ASIAN AMERICAN STUDIES"
)

dept.deg.df <- dept.deg.df[ ! dept.deg.index.to.exclude, ]



enrollment.df <- merge(enrollment.df, dept.deg.df[, c("PLAN_DESCR", "Department.Name", "Discipline.Area.Descr")], all.x=TRUE)
...
employment.df <- employment.df[ order(employment.df$Uw.Dv.Job.Fte, employment.df$Uw.Jobcode.Descr, decreasing = TRUE), ]
# So the highest FTE is considered your "first" appointment
# Then sort by job code description just to have some consistency if there are ties.
...
ordinal.names <- c("First", "Second", "Third", "Fourth", "Fifth", "Sixth", "Seventh", "Eighth", "Ninth", 
                   "Tenth", "Eleventh", "Twelfth", "Thirteenth", "Fourteenth", "Fifthteenth")

for ( i in 2:length(multi.appts.ls)) {

colnames(multi.appts.ls[[i]])[colnames(multi.appts.ls[[i]]) %in% mult.appt.columns] <- 
  paste0(colnames(multi.appts.ls[[i]])[colnames(multi.appts.ls[[i]]) %in% mult.appt.columns], ".", ordinal.names[i])

employment.df <- merge(employment.df, 
    multi.appts.ls[[i]][, c("Emplid", paste0(mult.appt.columns, ".", ordinal.names[i]))], all.x=TRUE)
...
names(multi.program.df)[names(multi.program.df)=="Department.Name"] <- "Department.Name.1"
names(multi.program.df)[names(multi.program.df)=="Discipline.Area.Descr"] <- "Discipline.Area.Descr.1"
...
while (nrow(enrollment.multi.processing.df) > 0) {
  ...
  names(enrollment.multi.processing.df)[ 
    grepl("Department.Name", names(enrollment.multi.processing.df))] <- 
    paste0("Department.Name.", i)
  names(enrollment.multi.processing.df)[ 
    grepl("Discipline.Area.Descr", names(enrollment.multi.processing.df))] <- 
    paste0("Discipline.Area.Descr.", i)
  ...
  multi.program.df <- merge(multi.program.df, enrollment.multi.processing.df[
    ...
    c("EMAIL_ADDRESS", paste0("PRIMARY_ACADEMIC_GROUP.", i), paste0("PLAN_DESCR.", i),
      paste0("Department.Name.", i), paste0("Discipline.Area.Descr.", i) )], all=TRUE)
...
multi.program.df$Department.Name.combined <- 
  do.call( paste3.pipe, multi.program.df[, paste0("Department.Name.", 1:(i-1))])
multi.program.df$Discipline.Area.Descr.combined <- 
  do.call( paste3.pipe, multi.program.df[, paste0("Discipline.Area.Descr.", 1:(i-1))])
...
enrollment.after.dedup.df$department_for_match <- enrollment.after.dedup.df$Department.Name.1
...
enrollment.after.dedup.df$age.EN <- as.period(new_interval(enrollment.after.dedup.df$BIRTHDATE.formatted.EN, as.Date(Sys.Date())))@year
# TODO: What is this warning message?: Warning message:
# In Ops.factor(left, right) : ‘-’ not meaningful for factors
# I can't tell what the problem is

enrollment.after.dedup.df$age.at.matriculation <- as.period(new_interval(start=enrollment.after.dedup.df$BIRTHDATE.formatted.EN, end=enrollment.after.dedup.df$matriculation.date))@year
...
write.csv(employment.after.dedup.df, file="/Users/travismcarthur/Desktop/TAA work/Grad student database/Employment final database version 12-7-15.csv", row.names=FALSE, fileEncoding="Latin1")

write.csv(membership.after.dedup.df, file="/Users/travismcarthur/Desktop/TAA work/Grad student database/Membership final database version 12-7-15.csv", row.names=FALSE, fileEncoding="Latin1")

write.csv(enrollment.after.dedup.df, file="/Users/travismcarthur/Desktop/TAA work/Grad student database/Enrollment final database version 12-7-15.csv", row.names=FALSE, fileEncoding="Latin1")
...
country.code.df <- read.csv("/Users/travismcarthur/Desktop/TAA work/Grad student database/countrycodes.csv", stringsAsFactors = FALSE)
# Source: https://raw.githubusercontent.com/pudo/lobbytransparency/master/etl/countrycodes.csv
# https://datahub.io/dataset/iso-3166-1-alpha-2-country-codes
# https://datahub.io/dataset/iso-3166-1-alpha-2-country-codes/resource/7cd9a907-da04-4d2d-98cf-3ff793190ff4
...
country.code.df <- country.code.df[country.code.df$iso3!="" & country.code.df$iso2!="", ]

colnames(country.code.df)[colnames(country.code.df)=="iso3"] <- "HOME_COUNTRY_CODE.EN"
colnames(country.code.df)[colnames(country.code.df)=="iso2"] <- "HOME_COUNTRY_CODE.iso2.EN"

full.outer.merge.df <- merge(full.outer.merge.df, country.code.df[, c("HOME_COUNTRY_CODE.EN", "HOME_COUNTRY_CODE.iso2.EN")], all.x=TRUE)

full.outer.merge.df$HOME_COUNTRY_CODE.iso2.EN[is.na(full.outer.merge.df$HOME_COUNTRY_CODE.iso2.EN)] <- ""

URLencode.v <- Vectorize(URLencode)

gender.api.urls <- URLencode.v(paste0("http://api.namsor.com/onomastics/api/json/gender/", full.outer.merge.df$First.Name.0, "/", full.outer.merge.df$Last.Name.0, "/", full.outer.merge.df$HOME_COUNTRY_CODE.iso2.EN) )


gender.ls <- list()
length(gender.ls) <- length(gender.api.urls)

for ( i in 1:length(gender.api.urls)) {
  gender.ls[[i]] <- fromJSON(gender.api.urls[i])
  cat(i, "\n")
}


gender.df <- rbindlist(gender.ls)

# hist(gender.df$scale)
# hist(gender.df$scale[full.outer.merge.df$is.intl.student.EN])
# hist(gender.df$scale[!full.outer.merge.df$is.intl.student.EN])


names(gender.df)[names(gender.df)=="scale"] <- "gender.confidence.scale.0"
names(gender.df)[names(gender.df)=="gender"] <- "probable.gender.0"

gender.df <- as.data.frame(gender.df)

full.outer.merge.df <- cbind(full.outer.merge.df, gender.df[, c("probable.gender.0", "gender.confidence.scale.0")])




write.csv(full.outer.merge.df, file="/Users/travismcarthur/Desktop/TAA work/Grad student database/Outer merge Enr Emp Mem version 12-7-15.csv", row.names=FALSE, fileEncoding="UTF-8")
# fileEncoding="Latin1"
# , na=""

save(full.outer.merge.df, file="/Users/travismcarthur/Desktop/TAA work/Grad student database/Outer merge Enr Emp Mem version 12-7-15.Rdata")
